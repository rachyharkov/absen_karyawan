                ...
                <td> <textarea class="form-control" rows="3" name="alasan" id="alasan"
                    placeholder="Alasan"><?php echo $alasan; ?></textarea></td>
							...
							<tr>
								<td>Lampiran</td>
								<td>
									<input type="file" class="form-control" name="lampiran" id="lampiran" placeholder="Lampiran" required />
									<?php
	...
	$(document).ready(function() {
		...
		let select = $('#users_id').selectize({
			...
			load: function(query, callback) {
				...
				$.ajax({
					url: '<?php echo base_url('admin/sakit/get_user_by_name_or_username') ?>',
					type: 'GET',
					dataType: 'json',
					data: {
						q: query,
		...
		var selectize = select[0].selectize.lock();



		<?php
			$getdatausers = $this->db->get_where('tbl_users', ['id' => $this->session->userdata('userid')])->row();
